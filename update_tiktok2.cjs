...
if (startIdx !== -1) {
    ...
    let endStrLF = endStr.replace(/\r/g, '');
    let tab1Idx = content.indexOf('{/* TAB 1:');
    
    // The tiktok block ends exactly before the closing divs that precede TAB 1
    // We can just find the start of those closing divs.
    let searchContent = content.substring(startIdx, tab1Idx);
    
    // Match the tiktok block which ends with `))} </div> </div>`
    // Let's find the closing `))} \n </div> \n </div>`
    // Actually, look at the last `</div>` before the closing div tower
    let regex = /\{\/\* TIKTOK COMPACTO \*\/\}[\s\S]*?\{\s*title: "Trend da Empresa", views: "195K", growth: "\+5%" \}\r?\n\s*\]\.map\(\(post, i\) => \([\s\S]*?\}\)\)\r?\n\s*<\/div>\r?\n\s*<\/div>\r?\n\s*<\/div>/;
    
    if (regex.test(searchContent)) {
       content = content.replace(regex, tiktokCard);
       fs.writeFileSync('src/pages/Marketing.tsx', content, 'utf8');
       console.log("Success");
    } else {
       console.log("Regex didn't match within searchContent.");
    }
} else {
    console.log("Could not find TIKTOK COMPACTO");
}
