...
let startIdx = content.indexOf('{/* INSTAGRAM COMPACTO */}');
let endIdx = content.indexOf('{/* TIKTOK COMPACTO */}');

if (startIdx !== -1 && endIdx !== -1) {
    let blockToReplace = content.substring(startIdx, endIdx);
    content = content.replace(blockToReplace, newInstaCard + '\n\n                    ');
    fs.writeFileSync('src/pages/Marketing.tsx', content, 'utf8');
    console.log("Success! Replaced Insta card.");
} else {
