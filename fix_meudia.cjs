...
const startMarker = '{(user?.email === "mara@hotmail.com" || currentUserName.startsWith(';
const endMarker = '          {/* 4. Card de Desempenho';

const startIndex = content.indexOf(startMarker);
const endIndex = content.indexOf(endMarker);

if (startIndex !== -1 && endIndex !== -1) {
  const replacement = `{(user?.email === "livia@hotmail.com" || currentUserName.startsWith("Rog\\u01F8rio") || currentUserName.startsWith("Rogério")) ? (
        ...
        {(user?.email === "mara@hotmail.com" || currentUserName.startsWith("Rog\\u01F8rio") || currentUserName.startsWith("Rogério")) ? (
...
`;
  ...
  // Fix the unicode for Rogerio dynamically by replacing with the exact string from the file
  const originalRogerioStr = content.substring(startIndex).match(/currentUserName\.startsWith\("Rog[^"]+"\)/)[0];
  
  let finalReplacement = replacement.replace(/currentUserName\.startsWith\("Rog\\u01F8rio"\) \|\| currentUserName\.startsWith\("Rogério"\)/g, originalRogerioStr);
