...
// Locate the block
const startMarkerRegex = /\{\/\* 2\. Pipeline de Aprova\?\S*\s*\(Corporate Workflow\) \*\/\}/;
// Actually just replace exactly the div:
const startDiv = '<div className="lg:col-span-5 bg-[#0a0a0a] border border-white/5 rounded-2xl p-5 flex flex-col">';
// Wait, is it lg:col-span-5? Yes.
const endDiv = '</div>\n                  </div>\n                </div>\n\n                {/* KPIs & Performance (Dashboard Style) */}';
...
for (let i = 0; i < contentLines.length; i++) {
    if (contentLines[i].includes('2. Pipeline de Aprova')) {
        blockStartLine = i;
    }
    if (contentLines[i].includes('KPIs & Performance (Dashboard Style)')) {
        blockEndLine = i - 1; // back up slightly to the end of the previous div structure
        break;
    }
}

if (blockStartLine !== -1 && blockEndLine !== -1) {
    // Backtrack to find the exact closing tag of the Pipeline block
    // It's probably easier to just replace from blockStartLine to blockEndLine.
    let oldBlockStr = contentLines.slice(blockStartLine, blockEndLine).join('\n');
...
`;
    ...
    content = content.replace(oldBlockStr, newApprovalBlock);
}
...
console.log("Success! Pipeline implemented.");
