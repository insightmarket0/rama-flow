...
if (startIndex !== -1 && endIndex !== -1) {
    let appBlock = content.substring(startIndex, endIndex);
    
    // Find the original phone content inside appBlock
    const phoneStartIndex = appBlock.indexOf('<div className="relative w-[340px] h-[720px]');
    let phoneContent = '';
    
    if (phoneStartIndex !== -1) {
       let temp = appBlock.substring(phoneStartIndex);
    ...
    console.log('Successfully replaced dual phone block!');
} else {
