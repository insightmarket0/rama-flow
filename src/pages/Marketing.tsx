...
const INITIAL_SCRIPTS = [
  {
    ...
    date: "2h atrÃ¡s",
    description: "Criador focado em marcenaria vai demonstrar o torque e bateria do equipamento recÃ©m lanÃ§ado.",
    blocks: [
      { id: 1, title: "1. O Gancho (Hook - 0 a 3 seg)", icon: "FileText", color: "pink", content: "<p class='italic text-gray-500 mb-2'>// O criador recebe a caixa misteriosa. Ele abre com expressÃ£o de choque para a cÃ¢mera.</p>\"Eu duvidei que essa ferramenta aguentaria o tranco da minha marcenaria, mas olha o que veio nessa caixa da [Nome da Marca].\"" },
      { id: 2, title: "2. Desenvolvimento (Provas e Features)", icon: "PlayCircle", color: "cyan", content: "<ul class='list-disc list-inside space-y-2'><li><strong>Cena 1:</strong> Mostrar o torque furando uma madeira maciÃ§a (Angulo fechado na broca).</li><li><strong>Cena 2:</strong> Mostrar a bateria (Mencionar que dura 20h direto).</li><li><strong>Fala:</strong> \"O torque disso aqui Ã© absurdo. Ela atravessa peroba rosa como se fosse manteiga, e a bateria nÃ£o me deixa na mÃ£o no meio do projeto.\"</li></ul>" },
      { id: 3, title: "3. Call to Action (CTA)", icon: "ShoppingBag", color: "emerald", content: "<p class='italic text-gray-500 mb-2'>// Apontar para baixo (link da bio/carrinho) segurando a ferramenta.</p>\"Se vocÃª Ã© marceneiro ou faz DIY, isso aqui Ã© investimento. Clica no link da minha bio que eu consegui um cupom de 15% de desconto lÃ¡ na loja deles na Shopee!\"" }
    ]
  },
  {
    id: 2,
    ...
    date: "Ontem",
    description: "Ideia rÃ¡pida para um hook chamativo.",
    blocks: []
  },
  {
    id: 3,
                        ...
                        <td className="px-4 py-2">
                          <select
                            ...
                            className={`appearance-none cursor-pointer outline-none inline-flex items-center text-[10px] font-medium px-2 py-0.5 rounded border transition-colors ${
                              ...
                              partner.status === 'AprovaÃ§Ã£o Interna' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20 hover:bg-blue-500/20' :
                              partner.status === 'Aguardando Roteiro' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20 hover:bg-purple-500/20' :
                              partner.status === 'RefaÃ§Ã£o' ? 'bg-red-500/10 text-red-400 border-red-500/20 hover:bg-red-500/20' :
                              'bg-gray-500/10 text-gray-400 border-gray-500/20 hover:bg-gray-500/20'
                            }`}
                          >
                            <option value="Postado" className="bg-[#111] text-emerald-400">Postado</option>
                            <option value="AprovaÃ§Ã£o Interna" className="bg-[#111] text-blue-400">AprovaÃ§Ã£o Interna</option>
                            <option value="Aguardando Roteiro" className="bg-[#111] text-purple-400">Aguardando Roteiro</option>
                            <option value="RefaÃ§Ã£o" className="bg-[#111] text-red-400">RefaÃ§Ã£o</option>
                          </select>
                        </td>
              ...
              <div className="flex flex-col gap-3 h-full">
                ...
                <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden group hover:border-white/10 transition-colors flex-1 min-h-[150px]">
                  ...
                  <div className="flex justify-between mb-2 items-center relative z-10">
                    ...
                    <button onClick={handleEditSales} className="bg-white/5 hover:bg-white/10 text-gray-400 px-3 py-1 rounded-full text-[9px] font-medium transition-colors border border-white/5 uppercase flex items-center gap-1">LanÃ§ar +</button>
                  </div>
...
{/* TIKTOK COMPACTO */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6 relative z-10 w-full max-w-sm shadow-2xl flex flex-col gap-4">
            ...
            <p className="text-[10px] text-center text-gray-500 mt-2">
              Ao aprovar, o valor de R$ {approvalDetails.amount.toLocaleString('pt-BR')} serÃ¡ adicionado automaticamente ação "Gasto" do seu OrÃ§amento atual.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl w-full max-w-md flex flex-col shadow-2xl">
            ...
            <div className="p-6 space-y-5">
              <div>
                <label className="text-[9px] uppercase font-bold tracking-widest text-gray-500 mb-1 block">TÃ­tulo da EstratÃ©gia</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="text-[9px] uppercase font-bold tracking-widest text-gray-500 mb-1 block">DescriÃ§Ã£o do Roteiro (Opcional)</label>
                <textarea 
                  value={newScriptForm.description} 
                  onChange={(e) => setNewScriptForm({...newScriptForm, description: e.target.value})} 
                  placeholder="Ex: Roteiro focado em mostrar os benefÃ­cios..."
                  ...
                  {ASSETS.map(asset => (
                    <div key={asset.id} className="bg-[#111] border border-[#222] rounded-lg p-3 flex flex-col cursor-pointer hover:border-[#444]">
                      ...
                      <div className="mt-auto">
                        ...
                        <p className="text-[9px] text-gray-500 mt-0.5">{asset.type} Ã¢Â¬Â¢ {asset.size}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {isCrmModalOpen && editingCrmPartner && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl w-full max-w-2xl flex flex-col shadow-2xl">
            
            <div className="p-5 flex justify-between items-center">
              <h2 className="text-white font-semibold text-lg tracking-tight">Editar Parceiro</h2>
              <button onClick={() => setIsCrmModalOpen(false)} className="text-gray-500 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            
            <div className="px-6 pb-2 overflow-y-auto max-h-[70vh] custom-scrollbar space-y-5">
              {/* Profile Image Section */}
              <div className="flex gap-4 items-center">
                <Avatar className="w-16 h-16 rounded-full border border-[#222]">
                  <AvatarImage src={editingCrmPartner.avatar} className="object-cover" />
                  <AvatarFallback className="bg-[#1a1a1a] text-white rounded-full text-lg font-bold">?</AvatarFallback>
                </Avatar>
                <div className="flex-1 space-y-1.5">
                  <div>
                    <h3 className="text-white text-sm font-medium tracking-tight">Foto do Perfil</h3>
                    <p className="text-[10px] text-gray-500">Recomendado: 400x400px</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <input 
                      type="file" 
                    ...
                    />
                    <label htmlFor="avatar-upload" className="cursor-pointer bg-white/5 hover:bg-white/10 border border-white/5 transition-colors rounded-lg text-gray-300 text-xs px-3 py-1.5 flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                ...
                <div>
                  ...
                  <select value={editingCrmPartner.status} onChange={(e) => setEditingCrmPartner({...editingCrmPartner, status: e.target.value})} className="w-full bg-[#111] border border-white/5 rounded-lg text-white text-sm px-3 py-2 focus:border-cyan-500/50 focus:bg-[#161616] transition-colors outline-none appearance-none cursor-pointer">
                    ...
                    <option value="AprovaÃ§Ã£o Interna">AprovaÃ§Ã£o Interna</option>
                    <option value="Aguardando Roteiro">Aguardando Roteiro</option>
                    <option value="RefaÃ§Ã£o">RefaÃ§Ã£o</option>
                  </select>
                </div>
                <div>
                ...
                <div>
                  <label className="text-[9px] uppercase font-bold tracking-widest text-gray-500 mb-1 block">ExpiraÃ§Ã£o (Dias)</label>
            ...
            <div className="p-5 flex justify-end gap-3 mt-2">
              ...
              <button onClick={handleSaveCrmPartner} className="px-5 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-sm font-semibold transition-colors shadow-lg">Salvar AlteraÃ§Ãµes</button>
            </div>
          </div>
        </div>
      )}
