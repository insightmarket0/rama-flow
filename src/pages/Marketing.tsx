        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6 relative z-10 w-full max-w-sm shadow-2xl flex flex-col gap-4">
            ...
            <p className="text-[10px] text-center text-gray-500 mt-2">
              Ao aprovar, o valor de R$ {approvalDetails.amount.toLocaleString('pt-BR')} será adicionado automaticamente a��o "Gasto" do seu Orçamento atual.
            </p>
          </div>
        </div>
      )}
              ...
              <button 
                onClick={() => {
                  const newScript = {
                    ...
                    blocks: [
                      { id: Date.now() + 1, title: "1. O Gancho (Hook - 0 a 3 seg)", icon: "FileText", color: "pink", content: "<p class='italic text-gray-500 mb-2'>// Escreva aqui a a��o inicial ou visual chamativo.</p>\"Sua fala principal do gancho aqui...\"" },
                      { id: Date.now() + 2, title: "2. Desenvolvimento (Provas e Features)", icon: "PlayCircle", color: "cyan", content: "<ul class='list-disc list-inside space-y-2'><li><strong>Cena 1:</strong> Mostre o produto em uso...</li><li><strong>Fala:</strong> \"Explique o principal benef�cio aqui...\"</li></ul>" },
                      { id: Date.now() + 3, title: "3. Call to Action (CTA)", icon: "ShoppingBag", color: "emerald", content: "<p class='italic text-gray-500 mb-2'>// Aponte para o link ou produto.</p>\"Chamada final para a��o, ex: Clica no link da bio!\"" }
                    ]
                  };
                  ...
                  {ASSETS.map(asset => (
                    <div key={asset.id} className="bg-[#111] border border-[#222] rounded-lg p-3 flex flex-col cursor-pointer hover:border-[#444]">
                      ...
                      <div className="mt-auto">
                        ...
                        <p className="text-[9px] text-gray-500 mt-0.5">{asset.type} â¬¢ {asset.size}</p>
                      </div>
                    </div>
                  ))}
                </div>
