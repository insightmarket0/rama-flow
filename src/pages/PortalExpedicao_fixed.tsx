...
export default function PortalExpedicao() {
  ...
  const handleQuickPing = (type: string) => {
    switch (type) {
      case 'supervisor':
        toast("Chamando supervisor...", { icon: "⚠️" });
        break;
      case 'falta_caixa':
        toast("Aviso de falta de caixa enviado.", { icon: "📦" });
        break;
      case 'pausa_fluxo':
        toast("Fluxo pausado temporariamente.", { icon: "⏳" });
        break;
      case 'tudo_ok':
        toast("Status atualizado: Tudo OK.", { icon: "✅" });
        break;
    }
  };
  ...
  const [activeTab, setActiveTab] = useState<"kanban" | "novo_chamado" | "historico">("kanban");
          ...
          <div className="p-3 bg-[#181818] border-t border-white/5">
            ...
            <form onSubmit={handleChatSubmit} className="relative mt-2">
              <input 
                ...
                className="hidden" 
              />
              <div className="flex items-center bg-white/[0.02] backdrop-blur-md rounded-full border border-white/[0.05] focus-within:bg-white/[0.05] focus-within:border-[#00FF00]/50 focus-within:shadow-[0_0_20px_rgba(0,255,0,0.1)] transition-all pr-1.5 pl-4 h-12">
                <input 
                  type="text"
                  value={chatMessage}
