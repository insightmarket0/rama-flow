...
const splitStart = '<div className="lg:col-span-7 bg-[#0a0a0a] border border-white/5 rounded-2xl p-5 flex flex-col">';
const splitEnd = '{/* 2. Pipeline de Aprova';

let sIdx = content.indexOf(splitStart);
let eIdx = content.indexOf(splitEnd, sIdx);
...
if (sIdx !== -1 && eIdx !== -1) {
    ...
    content = content.replace(block, newSplitCard);
    fs.writeFileSync('src/pages/Marketing.tsx', content, 'utf8');
    console.log("Success! Split card restored.");
} else {
