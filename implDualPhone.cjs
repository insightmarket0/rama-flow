...
const startStr = '{/* Simula';
const endStr = '{activeTab === "ecossistema" && (';
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    // We want to extract the inner content of the phone, or we can just keep the whole phone string
    // block contains the original phone.
    // It starts with `{/* Simula... */}\n <div className="relative...`
    // And ends with `\n </div>\n </div>\n )}\n`
    
    // Let's replace the single phone container with our dual phone layout:
    let newBlock = `
...
` + 
    block.substring(block.indexOf('<div className="relative w-[340px] h-[720px]'), block.lastIndexOf('</div>', block.lastIndexOf('</div>', block.lastIndexOf('}') - 1) - 1) + 6) 
    + `
                </div>

                {/* VS Badge */}
                <div className="hidden xl:flex w-12 h-12 rounded-full bg-[#111] border-2 border-white/10 items-center justify-center z-10 shrink-0">
                ...
                <div className="flex flex-col gap-4 items-center">
                  ...
                  <div className="relative w-[340px] h-[720px] bg-[#0b141a] rounded-[3rem] border-[8px] border-[#1A1A1A] shadow-2xl overflow-hidden flex flex-col ring-1 ring-white/10 shrink-0 font-sans">
                    ...
                    <div className="bg-[#202c33] w-full pt-12 pb-3 px-4 flex items-center gap-3 z-30 shadow-sm relative">
                      <svg viewBox="0 0 24 24" className="text-[#8696a0] w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                      ...
                      <div className="flex flex-col flex-1 leading-tight">
                        <div className="flex items-center gap-1">
                          <span className="text-[#e9edef] font-bold text-base">Rama Flow</span>
                        ...
                        <span className="text-[#8696a0] text-[11px]">Atendimento Oficial</span>
                      </div>
                    </div>
    ...
    console.log('Dual phone implementation successful!');
} else {
