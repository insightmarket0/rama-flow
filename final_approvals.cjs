...
const startMarker = '<div className="lg:col-span-5 bg-[#0a0a0a]';
let startIdx = content.indexOf(startMarker, content.indexOf('Aprova'));

if (startIdx !== -1) {
    let innerText = 'Escala de Criativo #04';
    let innerIdx = content.indexOf(innerText, startIdx);
    if (innerIdx !== -1) {
        let div1 = content.indexOf('</div>', innerIdx); 
        let div2 = content.indexOf('</div>', div1 + 6); 
        let div3 = content.indexOf('</div>', div2 + 6); 
        let div4 = content.indexOf('</div>', div3 + 6); 
...
`;
        ...
        console.log("Success! Pipeline implemented correctly.");
    } else {
