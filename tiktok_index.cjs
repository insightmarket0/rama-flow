...
if (startIdx !== -1 && endIdx !== -1) {
    let middle = content.substring(startIdx, endIdx);
    
    // The exact string the tiktok card ends with:
    let endOfTiktokRegex = /\}\)\)\s*<\/div>\s*<\/div>\s*<\/div>/;
    let match = middle.match(endOfTiktokRegex);
    if (match) {
        let actualEndOfTikTokBlock = middle.indexOf(match[0]) + match[0].length;
        let blockToReplace = middle.substring(0, actualEndOfTikTokBlock);
        
        content = content.replace(blockToReplace, newTiktokCard);
        ...
        console.log("Success! Index replacement worked.");
    } else {
        console.log("Could not find end of tiktok block using indexOf");
    }
}
