...
const targetStr = `                  <span className="text-[10px] uppercase tracking-[0.2em] font-bold">
                    {mk.status === "completed" ? (
...
const newBlock = `                  <span className="text-[10px] uppercase tracking-[0.2em] font-bold">
                    {mk.status === "completed" ? (
                      <span className="text-[#CCFF00]">ATIVO</span>
                    ) : (
                      <span className="text-orange-500">A FAZER</span>
        ...
        <section className="bg-[#111111] border-4 border-white/20 p-6 lg:p-10 shadow-2xl relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border-4 border-white/20">
              ...
              <div className="bg-[#111111] p-6 border-b-4 md:border-b-0 md:border-r-4 border-white/20 hover:bg-white/5 transition-colors">
                ...
                <p className="text-white text-xs leading-relaxed font-bold">
                  Segunda conta estadual com <strong>parceiro operador local</strong> que cuida de toda a logística. A RAMA atua como distribuidora matriz, <span className="text-[#CCFF00]">lucrando direto na fonte (fornecimento)</span> e garantindo escalabilidade para o Sul.
                </p>
              ...
              <div className="bg-[#050505] p-6 hover:bg-white/5 transition-colors">
                ...
                <p className="text-gray-500 text-xs leading-relaxed font-bold">
                  Após estabelecer com sucesso o modelo de distribuição no Sul, replicaremos a estrutura abrindo o próximo polo operacional para atingir e dominar a região Nordeste.
                </p>
...
if (content.includes(targetStr)) {
    ...
    console.log('Expansion plan added successfully!');
} else {
    console.log('Target block not found. Checking line endings issues...');
    // Sometimes whitespace or exact formatting prevents match, let's use a simpler marker
}
