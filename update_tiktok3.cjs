...
if (startIdx !== -1 && endIdx !== -1) {
    ...
    let blockToReplace = content.substring(startIdx, endIdx + endStr2.length);
    content = content.replace(blockToReplace, tiktokCard);
    
    // Also fix the PlayCircle import if we use FaTiktok
    if (!content.includes('FaTiktok')) {
        content = content.replace(/(import .*? from "lucide-react";)/, `$1\nimport { FaTiktok } from "react-icons/fa";`);
    }
    // Replace PlayCircle with FaTiktok inside modal if necessary
