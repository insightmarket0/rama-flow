...
// Insert the new member right after Mara's closing brace.
const maraEndStr = 'children: []\n      }';
const insertIndex = content.lastIndexOf(maraEndStr);

if (insertIndex !== -1) {
    const splitIndex = insertIndex + maraEndStr.length;
    content = content.substring(0, splitIndex) + ',\n' + newMember + content.substring(splitIndex);
    fs.writeFileSync(file, content, 'utf8');
    console.log('Added Tecnico Gas!');
} else {
    console.log('Could not find Mara end string');
}
