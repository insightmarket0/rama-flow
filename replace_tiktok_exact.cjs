...
if (startIdx !== -1) {
    ...
    let mapCloseIdx = searchArea.indexOf('))}');
    let afterMapClose = searchArea.substring(mapCloseIdx);
    
    // find the 3rd </div>
    let div1 = afterMapClose.indexOf('</div>');
    let div2 = afterMapClose.indexOf('</div>', div1 + 6);
    let div3 = afterMapClose.indexOf('</div>', div2 + 6);
    
    if (div3 !== -1) {
        let exactBlockLength = mapCloseIdx + div3 + 6;
        let blockToReplace = searchArea.substring(0, exactBlockLength);
        ...
        console.log("Success! Replaced tiktok exactly.");
    } else {
