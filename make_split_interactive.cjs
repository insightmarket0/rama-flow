...
// 2. Modify the Split card to have the range inputs and remove the "Lançar +" button.
const oldSplitStart = '<div className="lg:col-span-7 bg-[#0a0a0a] border border-white/5 rounded-2xl p-5 flex flex-col relative overflow-hidden group">';
const oldSplitEnd = '{/* 2. Meta vs Realizado */}';

let startIdx = content.indexOf(oldSplitStart);
let endIdx = content.indexOf(oldSplitEnd);
                    ...
                    <div className="space-y-6 mt-auto relative z-10">
                      <div>
                        ...
                        <div className="relative w-full h-3 bg-[#1a1a1a] rounded-full overflow-hidden hover:bg-[#222] transition-colors cursor-ew-resize">
                          <div className="h-full bg-cyan-500 transition-all duration-75" style={{ width: \`\${marketingBudget.budgetSplit?.trafego || 0}%\` }}></div>
                          <input 
                      ...
                      <div>
                        ...
                        <div className="relative w-full h-3 bg-[#1a1a1a] rounded-full overflow-hidden hover:bg-[#222] transition-colors cursor-ew-resize">
                          <div className="h-full bg-purple-500 transition-all duration-75" style={{ width: \`\${marketingBudget.budgetSplit?.influenciadores || 0}%\` }}></div>
                          <input 
                            type="range" min="0" max="100" 
                      ...
                      <div>
                        ...
                        <div className="relative w-full h-3 bg-[#1a1a1a] rounded-full overflow-hidden hover:bg-[#222] transition-colors cursor-ew-resize">
                          <div className="h-full bg-orange-500 transition-all duration-75" style={{ width: \`\${marketingBudget.budgetSplit?.seeding || 0}%\` }}></div>
                          <input 
                            type="range" min="0" max="100" 
...
`;
    ...
    console.log("Success! Split card is now interactive.");
} else {
    console.log("Could not find start or end of split card.");
}
