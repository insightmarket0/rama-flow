...
// MOVE RADAR AND TERMOMETRO
const radarStart = content.indexOf('{/* Radar Logístico (Esquerda, 4 colunas) */}');
const radarEnd = content.indexOf('{/* --- NOVA LINHA DO COCKPIT --- */}');

if (radarStart !== -1 && radarEnd !== -1) {
  const radarBlock = content.substring(radarStart, radarEnd);
  content = content.substring(0, radarStart) + content.substring(radarEnd);
  
  const perfStr = '{/* TAB 3: PERFORMANCE */}';
  const perfIdx = content.indexOf(perfStr);
  if (perfIdx !== -1) {
    const afterPerf = content.indexOf('<div className="flex-1 flex flex-col gap-3 min-h-0">', perfIdx);
    if (afterPerf !== -1) {
       const insertPos = afterPerf + '<div className="flex-1 flex flex-col gap-3 min-h-0">'.length;
...
// COMPACT TRACKER
const searchStr = '{/* Social Media Tracker (Direita, 4 colunas) */}';
const startIdx = content.indexOf(searchStr);
const endMarker = '{/* TAB 1: CREATIVE STUDIO (Master-Detail / Notion Style) */}';
let endIdx = content.indexOf(endMarker, startIdx);

if (startIdx !== -1 && endIdx !== -1) {
  // find the correct closing div before endMarker
  const closingDivs = `              </div>\n            </div>\n          )}\n\n          `;
  endIdx = endIdx - closingDivs.length;
  ...
  content = content.substring(0, startIdx) + sleekTrackers + content.substring(endIdx);
} else {
  console.log("Could not find start or end index for sleek tracker");
}
