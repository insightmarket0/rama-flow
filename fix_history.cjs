...
// 2. Change grid-cols-4 to grid-cols-2
content = content.replace('<div className="grid grid-cols-4 gap-3 shrink-0">', '<div className="grid grid-cols-2 gap-3 shrink-0">');

// 3. Re-inject Social media blocks if they are missing
const orcamentoBlockRegex = /<div className="grid grid-cols-2 gap-3 shrink-0">([\s\S]*?)<\/div>\s*<\/div>\s*\)\}/;
const match = content.match(orcamentoBlockRegex);

if (match) {
    let oldCards = match[1];
    
    // 4. Update the Budget card to include the History toggle
    ...
    console.log('Restored isHistoryExpanded logic!');
} else {
    console.log('Could not find ORCAMENTO tab block to patch');
}
